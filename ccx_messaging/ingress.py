...
def parse_identity(encoded_identity: bytes) -> dict:
    """Generates a dictionary object from a base64 encoded input."""
    try:
        ...
        raise CCXMessagingError(
            f"Unable to decode received message: {decoded_identity}"
        ) from ex

    except jsonschema.ValidationError as ex:
        raise CCXMessagingError(
            f"Invalid input message JSON schema: {identity}"
        ) from ex


def parse_ingress_message(message: bytes) -> dict:
    """Parses a bytes messages into a dictionary, decoding encoded values."""
    try:
